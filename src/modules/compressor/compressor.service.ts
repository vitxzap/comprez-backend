...
export class CompressorService {
  constructor(
    private readonly compressorContract: CompressorContract,
    private readonly featureFlag: FeatureFlagService,
    private readonly s3Service: S3Service
  ) {}
  private logger = new Logger(CompressorService.name);
  async requestS3Upload(
    params: RequestS3UploadDto,
    userId: string
  ): Promise<S3UploadResponseDto> {
    if (
      await this.featureFlag.isCachedFlagEnabled(
        FEATURE_FLAGS.ENABLE_S3_FEATURES
      )
    ) {
  ...
  async requestS3Download(
    ...
    if (
      ...
      //Generates the key (path) for the s3
      const key = await this.compressorContract.getS3KeyById(
        userId,
        compressionId
      );
  ...
  async getUserCompressions(userId: string): Promise<UserCompressions[]> {
    const compressions =
      await this.compressorContract.getUserCompressionsById(userId);
    return compressions;
  }
}
