...
export class FeatureFlagService extends Flagsmith {
  ...
  async isCachedFlagEnabled(flag: FEATURE_FLAGS) {
    const isFlagEnabled = await this.getCachedFlags().then((features: Flags) =>
      features.isFeatureEnabled(flag)
    );
  ...
  async getCachedFlagValues(flag: FEATURE_FLAGS) {
    const flagValues = await this.getCachedFlags().then((features: Flags) =>
      features?.getFeatureValue(flag)
    );
    return flagValues;
  }
}
